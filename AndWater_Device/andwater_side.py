...
MERGED_BLOCKS_SIDE = [
    {"row": 1, "col": 0, "row_span": 2, "col_span": 2}  # G 大灯箱
]
...
N_KEYS_SIDE = len(KEYS_ORDER_SIDE)  # = 21，整数常量
...
def accelerate_side_mapping(
    ...
    box_max_degrees = np.zeros(N_KEYS_SIDE, dtype=np.int32)
    box_counts = np.zeros(N_KEYS_SIDE, dtype=np.int32)
    ...
    for i in range(n_points):
        ...
        if z <= Z_MAX:
            ...
            if (X_MIN <= x_scaled <= X_MAX) and (Y_MIN <= y_scaled <= Y_MAX):
                ...
                row_6x4 = int((Y_MAX - y_scaled) / (Y_MAX - Y_MIN) * 6)
                
                if col_6x4 < 0: col_6x4 = 0
                if col_6x4 > 3: col_6x4 = 3 # 侧面只有 4 列
                if row_6x4 < 0: row_6x4 = 0
                if row_6x4 > 5: row_6x4 = 5

                box_idx = SIDE_GRID_INDEX[row_6x4, col_6x4]
...
def main():
    ...
    ser = None
    try:
        ser = serial.Serial('/dev/ttyUSB1', 115200) # 侧面墙使用 /dev/ttyUSB1
        print("Teensy 侧面专线串口已连接")
    except Exception as e:
        print(f"Teensy 侧面串口未连接（本地预览开启）")

    # 滤波器、施密特、零延迟跳帧限频器初始化
    last_box_degrees = {k: 0.0 for k in KEYS_ORDER_SIDE} # 侧面墙专用
    box_active_states = {k: False for k in KEYS_ORDER_SIDE} # 侧面墙专用
    ...
    try:
        while True:
            ...
            # 🚀 性能大突破：调用 Numba 编译后的函数。直接使用 [::1]（无损全分辨率），CPU 单核消耗仍会降至接近 0%！
            ...
            for key in KEYS_ORDER_SIDE:
                ...
                avg_degree = total_sum # 此时 avg_degree 已经是最大值，不再是平均值
