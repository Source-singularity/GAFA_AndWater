...
- Normal 1 x 1 light boxes are wired by column and use the PlanA-style column PWM outputs.
- Blue merged 2 x 2 strip blocks are separate LED strips. Their wiring is not fixed yet, so the firmware keeps them as reserved logic units and does not fold them into the light-box column outputs.
